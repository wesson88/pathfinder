...
- **前端厚、云端薄**：计分、报告生成全在本地（纯函数 `computeResult`），云端只做身份归属、订单真相、存档。
- **幂等交卷**：报告 `_id = sessionId`，云端事务内复核订单 + 条件核销，冲突重试 ≤3（`submitTest`）。
...
1. `cloud/functions/createOrder|checkOrder` 配置 `WECHAT_MCH_ID`（商户号）；`payCallback` 接入签名校验
